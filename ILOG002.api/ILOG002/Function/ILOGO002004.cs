...
using ILOG002.Models._004.Request;
using ILOG002.Models._004.Response;
...
namespace IPRO002
{
    public static class ILOGO002004
    {
        ...
        public static async Task Run([ServiceBusTrigger("%QueueRequest004%", Connection = "ConectionStringRequest")] string myQueueItem, ILogger log)
        {
            try
            {
                Logger.FileLogger("APILOGO002004", "Procesando Función");

                ResponseHomologacion respuesta = null;

                var APILOG002004Request = JsonConvert.DeserializeObject<APILOG002004MessageRequest>(myQueueItem);

                //Homologación
                string DataAreaId = APILOG002004Request.DataAreaId;
                ...
                APILOG002004Request.DataAreaId = respuesta?.Response ?? "000001";
                ////////////////////////////////////////////////////////////////
                ...
                Logger.FileLogger("APILOGO002004", "Resultado:" + objResponse.DescripcionId);
                //
                APILOG002004MessageResponse APILOG002004Response = new APILOG002004MessageResponse();
                string sesionid = APILOG002004Request.SessionId.ToString();
                //
                sesionid = Guid.NewGuid().ToString(); // hasta mientras, se debe borrar
                //
                APILOG002004Response.SessionId = sesionid;
                // falta mapeo resto de campos que responde WS

                IManejadorRequest<APILOG002004MessageResponse> _manejadorRequest = new ManejadorRequest<APILOG002004MessageResponse>();

                await _manejadorRequest.EnviarMensajeAsync(sesionid, sbConectionStringSend, sbQueueResponse, APILOG002004Response);


            }
            catch (Exception ex)
            {
                Logger.FileLogger("APILOGO002004", ex.ToString());
            }
